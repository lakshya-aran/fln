...
## 2026-09-01 — Bulk-class OCR end-to-end + diagnostic placement + report-view UI + analytics overlap fix

Seven commits covering four related areas: making the bulk-class OCR scan actually work end-to-end through the UI, fixing the diagnostic placement that was hardcoded to "level 2", exposing the saved report card to the teacher, and fixing the overlap on the Superadmin Geographical Analytics tab. Plus a per-question diagnostic log so the teacher/dev can correlate the UI verify table with the grading pass.

- **feat(ocr): extract printed student name per chunk in `/api/icr/evaluate-bulk`** — new `extractStudentNameFromDataUrl()` helper rasterizes only page 1 of each per-student sub-PDF and asks Gemma 4 to read the printed name from the top-left. Each bulk result now carries `studentName`, `studentNameError`, and `studentNameProcessingTimeMs`. Gracefully degrades when name extraction fails (the OCR answers still come through). Verified: `pagesPerStudent=2` chunk returns `studentName: "LAKSHYA ARAN"` for a 2-page Lakshya Aran paper.

- **feat(ocr): bulk-class OCR mode in scanner UI (`BulkIcrScan` + select step)** — `/api/icr/evaluate-bulk` existed in the backend but the frontend never called it (the single-sheet endpoint was being used to OCR a 22-page merged PDF, returning garbage). New `BulkIcrScan` component + a Single Sheet vs Bulk Class mode toggle in the ICR scanner's select step, a Pages Per Student input (default 2, configurable 1-10), a PDF-only uploader, and a new `bulk-select` step with a per-chunk card grid (clickable to jump to student) + a 42-row extracted-answer table for the selected student. Auto-matches chunk index → student by name (case-insensitive, whitespace-normalized) with chunk-index fallback; per-chunk student override picker for mis-matches. Lazy-loads each student's diagnostic paper from `/api/diagnostic/student/:id/answer-key` when their chunk is selected. "Save This Student's Report" + "Save All Reports" buttons post to `/api/students/:id/diagnostic/submit` (sequential so we don't hammer the backend).

- **feat(ocr): per-student verify table + bulk save reports + submit id-scheme fix** — new `ChunkVerifyTable` component (5-column verify table: row#, question prompt, correct answer from DB, editable OCR'd student answer, Correct/Incorrect badge). Falls back to OCR-only rows when no answer key exists. The diagnostic submit handler's answer-id remapping was extended to fall back to `diagnostic_answer_keys` when the `assignedDiagnosticQuestions` paper uses a different question-ID scheme than the answer-key record (a pre-existing data inconsistency that the bulk-OCR path was hitting as a 400). Frontend now uses the new `rows[]` from the answer-key endpoint (1 entry per printed row, pre-joined correctAnswer).

- **feat(answer-key): row-coordinate grouping via question_id+section parse** — the paper's `coords_mm` layout data is incomplete and unreliable (several sections like Match Shapes have no items[] entries; q-keys are question-numbers-within-section and shared across sections). The answer-key endpoint now parses the stable `Q_L20_<sec>_<q>[_b<blank>]` ID encoding directly. Multi-blank questions collapse to one row with comma-joined answers (e.g. `"2, 4, 7"` for a 3-blank Fill-in-Missing item). Fallbacks for paper-generator data bugs: Tens and Ones → `data.number`, Match Shapes → `from_label → to_label`, Compare Objects → `data.answer`. Verified: Vihaan Mishra's paper now returns 32 rows across all 10 sections, every row has a non-empty correctAnswer.

- **fix(diagnostic): deterministic placement + report-view UI** — the diagnostic submit handler used a brittle chain: Python pipeline → Gemini fallback → a placeholder formula. The Python pipeline routinely failed (503 retries), and Gemini's deterministic fallback mapped `recommendedLevel` to the lowest failed `source_level` which for Class 2 papers is almost always 2 regardless of how the student actually performed ("the student will be placed at level 2"). Replaced with a single deterministic scoring-based placement: `score` from per-question correctness, `recommendedLevel` = lowest failed `source_level` (Weakest-Level Mapping), `subLevel` = Mastery/Easier/Remedial within that level. All-correct → max source_level + 1, capped at 93. Plus a prominent placement-level callout in the single-flow result step (large gradient badge "Placed at Level L20.1" with sublevel + target next level) and "View Full Report" / "Past Reports" toggles so the teacher can actually see the saved report card + diagnostic history. Backend: fixed `GET /api/evaluation/:studentId/history` which was hanging at ~140k reports (full scan + JS filter) — now uses the store-layer `studentIds` filter. New `GET /api/students/:studentId/diagnostic-report` returns the most recent diagnostic placement for one student.

- **chore(diagnostic): log per-question verdict on submit** — one-to-one correspondence log on the backend submit handler. Header line: `[diag] <studentId> (<name>, Class N) — grading Q questions, S correct`. Then one line per question: `[diag]   <✓|✗|—> qid=Q_L...  submitted="..."  expected="..."  L<n>`. Verdicts: ✓ correct, ✗ wrong (non-empty submitted), — empty/missing. Lets the teacher/dev correlate the UI verify table with the grading pass. Also makes data-quality issues (e.g. Tens-and-Ones stored as "[object Object]", Match Shapes with empty expected answers) diagnostically visible — you can see exactly which questions have broken expected-answer data on a per-row basis.

- **fix(superadmin/analytics): resolve overlap on Geographical Analytics tab** — two layout fixes for the bottom card row in the Superadmin Geographical Analytics view (`RegionalAnalyticsView.tsx`). (1) The bar graph labels overlapped: `levelDistribution` data had 16 keys (Level 1..Level 16+) and the bar container renders one label per entry in a single flex row, so with 16 keys + `gap-3` the labels jammed together and visually overlapped the bar rectangles at the bottom. Now groups >8 entries into 6 buckets (L1-L3, L4-L6, L7-L9, L10-L12, L13-L15, L16+) so labels have room to breathe; ≤8-entry data still renders one bar per level. (2) The donut chart center text ("X% Certified") floated at the parent's top-left because the inner div had only `absolute text-center` (no `inset-0` or centering flex). Added `inset-0 flex flex-col items-center justify-center pointer-events-none` so it sits dead-center on the ring. Verified live on the Geographical Analytics tab: donut "0% Certified" now centered on the ring; bar graph shows 6 well-spaced bucket labels with no overlap with the bars above or the "Roster segment: N student profiles" line below.

- Files changed: `backend/src/routes/diagnosticBulk.ts` (340 lines), `backend/src/routes/evaluation.ts` (387 lines), `backend/src/routes/students.ts` (314 lines), `frontend/src/components/BulkIcrScan.tsx` (321 lines, new), `frontend/src/components/IcrScanner.tsx` (957 lines), `frontend/src/components/dashboards/RegionalAnalyticsView.tsx` (62 lines). 6 files changed, 2219 insertions(+), 183 deletions(-).

## 2026-08-30 — Question Intervention surface + late-August OCR plumbing

Two mostly-independent merges on `main` plus the OCR upload/size caps that were raised in earlier PRs (all PR'd but never added to CHANGELOG).

- **feat(superadmin): Question Intervention — author question logic per level** — new surface where the pedagogy team expresses *what to ask* at a given FLN level. A "question logic" is a natural-language instruction the generation pipeline turns into many concrete questions (not a question itself). The platform previously had nowhere to record that intent: the level generator and seed banks produce numbers, not pedagogy. Adds `QuestionLogic` on the `questionLogics` collection (with soft delete so deleted rows stay for audit), six Superadmin-only endpoints under `/api/question-logics` (including `/stats` for coverage counters and `/level-map` for dropdowns), and server-authoritative validation of every (level, skills, subskills) combination — the client's cascading dropdowns are a convenience, not a guard.

- **fix(prerequisites): apply 14 verified corrections + adjudicate 28 code-only hard prereqs** — two prerequisite-graph fixes. (1) PR #278: applies the 14 edges adjudicated in #277 as `required_for_procedure` with rationale, using the new per-edge `Prerequisite` object form where a level has mixed prerequisite strength: L8, L9, L16, L21, L22, L27, L28, L40, L42, L61, L64, L70, L84, L89. (2) PR #279: applies the surmise test to all 28 edges present only in the code graph (typed `required_for_procedure`) — 25 survive as hard (with rationale added to `skillProgressionMap.ts` and `Research/fln_level_networks.md` Part 4c), 3 are demoted to `often_precedes` (L32→L33 single-digit addition doesn't need place value, L66→L72 money arithmetic doesn't need 3-digit fluency, L82→L93 perimeter/area doesn't need full unit conversion). Follow-up: L63→L90 demoted from hard to `supports` per curriculum review — L90 is reading tenths/hundredths, which doesn't require flexible 3-digit decomposition. Apex-set sizes flagged as stale pending recompute.

- **docs: correct CLAUDE.md's stale auth-is-broken warning** — CLAUDE.md claimed auth was actively broken (plaintext Bearer-email role synthesis, unverified passwords, unauthenticated `/api/reset`). Verified directly against `backend/src/auth.ts` and `routes/auth.ts`: auth has since been hardened — real JWT signature verification, no email-prefix role synthesis (explicit in the code's own comment), real `bcrypt.compare()` at login, and `/api/reset` requires an authenticated superadmin. Every route file imports the same hardened `getAuthUser`, so there's no second implementation still carrying the old behavior. Also corrects the migration-rules line that gated mutation cutover on this same stale premise. Closes issue #396.

- **fix(ocr): raise Cloud OCR upload cap from 8 MB to 18 MB** (PR #319) — the 8 MB client-side limit was a leftover from the deleted `/api/icr/filter` + local-EasyOCR flow. The current backend rasterizes PDFs itself and only rejects above ~18 MB binary (24 MB base64), and nginx's `/fln` location allows 25 MB request bodies — so the frontend was rejecting legitimate PDF scans (e.g. 11.1 MB) that the backend could handle fine. Follow-up to PR #318 (which only handled oversized images via client-side compression and missed that PDFs bypass that path entirely).

- **fix(ocr): auto-compress oversized scans before Cloud OCR upload** (PR #318) — phone-camera scans routinely exceed the 8 MB cap at full resolution, so the "File too large" error was a dead end for users with no way to recover short of manually re-exporting. Downscale + re-encode as JPEG client-side (quality step-down, then dimension step-down) before the size check, so oversized-but-compressible scans go through automatically instead of failing.

- **fix(ocr): raise Cloud OCR page/size caps to support 22-25 page scans** (PR #320) — `MAX_PAGES=10` and `MAX_TOTAL_BASE64=24 MB` were both too tight for a real 22-page scan hit live in production. Raised `MAX_PAGES` to 25 and `MAX_TOTAL_BASE64` to 60 MB to match (sized for ~2.4 MB base64/page avg at 25 pages) so the page-count cap and the size cap don't just swap places on the same real-world file. Requires a corresponding nginx `client_max_body_size` bump on the `/fln` location (server-side config, not in this repo) to actually accept the larger request body — tracked separately as a deploy step.

- **fix(ocr): rasterize Cloud OCR PDF pages as JPEG @ 200 DPI, not lossless PNG @ 300 DPI** (PR #321) — root cause of the recurring "too large" errors on real multi-page scans. `pdf_rasterize.py` rendered pages at 300 DPI and saved as lossless PNG, which compresses photo-like scanned paper poorly. A real 22-page scan measured ~4.8 MB/page as PNG — far above the ~1-3 MB the size caps were tuned against — so raising `MAX_PAGES`/`MAX_TOTAL_BASE64` just kept chasing the same underlying problem. Switching to JPEG q85 @ 200 DPI (still well above what's needed for handwriting legibility) fixes page size at the source instead of re-tuning caps per incident. Also hardens two related edge cases: a single malformed/corrupt page no longer fails the entire multi-page batch (skipped and reported in `page_errors`); a stray poster/A0-sized page no longer renders at an uncapped resolution (longer side clamped to 3500 px).

## 2026-08-27 — Late-August Cloud OCR pipeline hardening (on `feat/bulk-ocr-name-extraction`)

Series of fixes raised the upload/size caps and added the bulk endpoint. All PR'd but never added to CHANGELOG.

- **feat(ocr): bulk endpoint splits multi-student PDF + richer fetch-fail diagnostics** — two related improvements. (1) New `POST /api/icr/evaluate-bulk` splits an uploaded multi-student PDF into per-student sub-PDFs (page count = `pagesPerStudent`, default 2 to match FLN Class 2-4 papers), then OCRs each sub-PDF sequentially via the existing `runCloudOcrOnImage` helper (pdf-lib for splitting, no logic duplication). Each sub-PDF is small enough to fit Ollama Cloud's per-request body limit, so a 25-student × 2-page = 50-page batch can now be scanned without hitting the 32 MB frontend / 100 MB Express / Ollama limits. Returns `{totalStudents, successfulStudents, failedStudents, results[]}` where each result has `{studentIndex, pageFrom, pageTo, pageCount, success, answers, rawOcrText, extractedTokens, processingTimeMs, error?, ocrEngine}`. Hard caps: 70 MB raw upload, 100 chunks max, 20 pagesPerStudent max. (2) Enriched fetch-fail catch block — was `'Cloud OCR failed: fetch failed'`. Node `fetch()` failures hide the underlying cause behind a generic 'fetch failed' message. Now extracts `e.code`, `e.cause.code`, `e.name` and returns `{error, errorCode, errorKind}` so the UI can tell DNS from TLS from timeout from ECONNRESET. Also logs the failure to the backend console with the stack top-3 for post-mortem. Status code changed from 500 to 502 (upstream failure, not server bug).

- **feat(ocr): raise upload size cap to 32 MB** — the frontend was hard-rejecting PDF/image uploads larger than 8 MB with "File too large: max 8 MB". The cap was our own (not Ollama's), and 8 MB is far too low for any realistic multi-page scan. `frontend/src/components/IcrTwoStageScan.tsx`: `MAX_UPLOAD_BYTES` raised from 8 MB to 32 MB. `backend/src/routes/evaluation.ts`: `MAX_TOTAL_BASE64` raised from 24 MB to 48 MB (about 32 MB binary). Both values stay in sync — 32 MB binary on the wire becomes about 48 MB base64 when JSON-encoded. This is our limit, not Ollama's. Ollama Cloud's `/api/chat` accepts much larger request bodies.

- **feat(ocr): raise `MAX_PAGES` from 10 to 50** — PDFs with more than 10 pages were rejected with a 400 from the backend, but a typical bulk batch (a class of 25 students × 2 pages each) is exactly 50 pages. The 10-page cap was too restrictive. `MAX_PAGES` raised from 10 to 50. `MAX_TOTAL_BASE64` raised from 48 MB to 96 MB. 50 pages at 300 DPI can produce ~50 MB base64; 96 MB gives headroom for high-resolution scans. The frontend's `MAX_UPLOAD_BYTES = 32 MB` cap stays as-is. Smoke test: 22-page PDF via pdf-lib returned 200 OK with Ollama returning `_meta.total_pages_received: 22`; 51-page PDF returned 400 "PDF has 51 pages; max is 50".

- **feat(ocr): raise `MAX_TOTAL_BASE64` from 96 MB to 256 MB** — a 22-page high-res scan produces ~141 MB base64, exceeding the previous 96 MB cap. The cap was too aggressive for real-world scans at 300 DPI. Raised to 256 MB (~170 MB binary). 256 MB covers ~22 high-res pages or ~50 lower-res pages. Frontend's 32 MB cap stays as-is and is checked first.

- **feat(ocr): lower PDF rasterization DPI from 300 to 150** — a 22-page high-res scan produces ~141 MB base64 at 300 DPI, which Ollama Cloud's `/api/chat` rejects with "fetch failed" (the request body exceeds Ollama's per-request limit). The earlier `MAX_TOTAL_BASE64=256 MB` cap was set too high — Ollama itself rejects the request before our cap fires. `ai-services/scripts/pdf_rasterize.py`: matrix DPI 300 → 150. Each A4 page goes from ~5-6 MB binary to ~1-1.5 MB. 22 pages now produce ~30-50 MB base64 — well within Ollama Cloud's limit. 150 DPI works for this OCR target because primary-school answer sheets have LARGE handwritten digits/numbers; Gemma 4 (vision model) reads 150 DPI handwriting fine.

- **fix(ocr): honest hint for Ollama 400/502 — NOT a bad key** — followed live traffic showing the API key was fine (5/5 successful curls in 5.6-8.1s right after a 400 screenshot). Ollama 400s in this app are intermittent rate limits or model-unavailable responses, not auth failures. Honest hints now: 503 unchanged; 502 + transport `errorCode` (ECONNRESET / ETIMEDOUT / ECONNREFUSED / EAI_AGAIN / ENOTFOUND) → "Ollama dropped or refused the connection (transport error). Retry usually helps..."; 502 + anything else → "Ollama Cloud rejected the request — could be a rate limit, the image was too large for Ollama's vision API, the model name is wrong, or the key was rotated..."; 400 → "Ollama Cloud returned a generic HTTP 400 — most often a per-minute rate limit or the model is temporarily unavailable. Retry usually works..."

- **fix(ocr): tailor 502 admin hint to actual errorCode (ECONNRESET vs unknown)** — backend now returns `errorCode` / `errorKind` on fetch-fail (e.g. ECONNRESET when Ollama Cloud drops the TCP connection mid-response), but the frontend was showing a generic "Ollama Cloud rejected the request — likely an invalid/revoked key, billing not enabled, or rate limit" hint for ALL 502s. That hint is wrong for transport errors: ECONNRESET means Ollama's edge proxy RST'd our connection mid-read, which is intermittent and not an auth/billing issue. Retrying usually works; chasing the API key would be a wild goose chase. Now the frontend tailors the hint based on `errorCode` — transport errors get the "retry helps" message, anything else gets the original auth/billing hint, with explicit 503 (no key configured) unchanged.

- **fix(ocr): de-duplicate 'Ollama Cloud:' prefix on bare HTTP errors** — user got 'Ollama Cloud: Ollama Cloud HTTP 400' because the wrapper string concatenated with its own fallback ('Ollama Cloud HTTP <status>') when Ollama's empty-body 400 response had no JSON `.error` key. Fix: build the fallback as just 'HTTP <status>' (no 'Ollama Cloud' in it), and add the 'Ollama Cloud: ' wrapper only when the chosen message doesn't already mention Ollama.
