...
## 2026-09-01 — Bulk-class OCR end-to-end + diagnostic placement + report-view UI

Six commits covering two related areas: making the bulk-class OCR scan actually work end-to-end through the UI, and fixing the diagnostic placement that was hardcoded to "level 2." Plus a per-question diagnostic log so the teacher/dev can correlate the UI verify table with the grading pass.
...
- Files changed: `backend/src/routes/diagnosticBulk.ts` (340 lines), `backend/src/routes/evaluation.ts` (387 lines), `backend/src/routes/students.ts` (314 lines), `frontend/src/components/BulkIcrScan.tsx` (321 lines, new), `frontend/src/components/IcrScanner.tsx` (957 lines). 5 files changed, 2157 insertions(+), 162 deletions(-).
